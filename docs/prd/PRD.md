...
### Propuesta IA
- LegacyPay debe ofrecer a los comercios autorizados una vista de historial de transacciones que permita filtrar y paginar sin exponer datos sensibles.
- La consulta debe mostrar solo información básica de la transacción necesaria para identificación y revisión.
- Los datos sintéticos se deben usar para ejemplos de prueba y demostraciones.
...
- Visualización de datos básicos de la transacción sin información sensible.
...
- El historial se limita a transacciones de los últimos 90 días.
- El listado debe soportar filtros por fecha, estado y monto.
- La consulta debe ser paginada.
- No se deben mostrar datos completos de tarjeta ni datos de autenticación.
...
- El listado contiene solo transacciones asociadas al comercio autenticado.
...
### Historia 5
Como comercio autorizado,
quiero ver solo mis transacciones,
para no acceder a información de otros comercios.

Criterios de aceptación:
- La consulta devuelve solo transacciones asociadas al comercio autenticado.
- No se incluyen datos de otro comercio en el resultado.
- No se exponen datos completos de tarjeta ni datos de autenticación.

## 5. Restricciones no funcionales
- Las consultas deben ser paginadas para evitar respuestas excesivamente grandes.
- La interfaz y/o API debe garantizar que ningún dato sensible se exponga.
...
- No se deben incluir datos completos de tarjeta, PAN completo, CVV ni credenciales de autenticación en ninguna respuesta.
...
- PREGUNTA ABIERTA: ¿Qué otros campos mínimos debe incluir cada transacción para identificación?
- PREGUNTA ABIERTA: ¿Se requieren controles adicionales para evitar que un comercio vea transacciones de otro comercio?